...
export function useSendMessage(conversationId: string, userId: string | undefined, didTheOptimistic: () => void) {
    ...
    const replaceTempMessage = (tempId: string, newMessage: MessageItemDto) => {
        queryClient.setQueryData<InfiniteData<MessagesListResponseDto>>(queryKey, (old) => {
            ...
            const pages = old.pages.map((page) => {
                const hasResolved = page.messages.some((item) => item.id === newMessage.id && item.id !== tempId);
    ...
    return useChatControllerSendMessage({
        mutation: {
            ...
            onError: (_err: unknown, _vars: unknown, context?: { previous?: unknown; tempId?: string }) => {
                if (context?.previous) {
